...
/**
 * @url https://github.com/k3d3/ed25519-java/blob/master/ed25519.java
 *
 * Written by k3d3
 * Released to the public domain
 */
public
class Ed25519_PureJava
{
	...
	static
	BigInteger expmod(BigInteger b, BigInteger e, BigInteger m)
	{
	...
	static
	BigInteger xrecover(BigInteger y)
	{
	...
	static
	BigInteger[] edwards(BigInteger[] P, BigInteger[] Q)
	{
		BigInteger x1 = P[0];
		BigInteger y1 = P[1];
		BigInteger x2 = Q[0];
	...
	static
	BigInteger[] scalarmult(BigInteger[] P, BigInteger e)
	{
	...
	public static
	byte[] signatureFrom512(byte[] m, byte[] h, byte[] pk)
	{
	...
	public static
	byte[] signature(byte[] m, byte[] sk, byte[] pk)
	{
		byte[] h = H(sk);
	...
	static
	boolean isoncurve(BigInteger[] P)
	{
		BigInteger x = P[0];
		BigInteger y = P[1];
	...
	static
	BigInteger[] decodepoint(byte[] s) throws Exception
	{
	...
	public static
	boolean checkvalid(byte[] s, byte[] m, byte[] pk) throws Exception
	{
		if (s.length != b / 4)
		{
