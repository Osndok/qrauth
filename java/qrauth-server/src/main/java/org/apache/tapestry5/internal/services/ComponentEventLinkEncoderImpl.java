...
/**
 * This is an exact clone of Tapestry's ComponentEventLinkEncoder except that the contextPath is
 * drawn from the request rather than the servlet context. We tried to use the "ServiceOverride"
 * mechanism, but that fails due to recursion; therefore we provide our own implementation
 * (with the same name and package) and rely on the container using our version (from the webapp's
 * classes directory) rather than the official version from the jar.
 */
