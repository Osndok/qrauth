...
** SESSION_ID (hashed)
** USER_IP
** {uid:12345, username:bob, alarm:false, security_rank:4, groups:{...}, permissions:[...]}
** returns either (1) the json login info, (2) the html that would allow this connection, or (3) if the session should be destroyed

* POST /api/tenant/logout
** API_KEY (unhashed)
** SESSION_ID (hashed)
** marks the session as obsolete (and depending on the user preferences, might logout all his/her sessions)
** not to be called automatically, or on idle timeout... only on user-request

* GET /user/account
* GET /api/tenant/user/account
** Not really an API call (no auth needed?)
** The first can be provided as a link (e.g. in a new window) to provide account maintenance
** The second will return an html blob which can be added to your page for account maintenance (an iframe)
